...
- **Attention:** The HC3 doesn't support dynamically creating audio clips for playback, so you'll need to create them in advance and host them in a public web based location where the Sonos player can access them. A great tool for generating pre-recorded commands is [GTTS (Google Text-to-Speech)](https://gtts.readthedocs.io/en/latest/cli.html). For example, my example audioclip is hosted on my public GitHub account.
